...
int main(int argc, char *argv[])
{
  ...
  for(;;) {
    fsize = sizeof(from);
    cc = recvfrom(socket_fd,(char *)Buffer,MAXLINE,MSG_WAITALL,(struct sockaddr *)&from,&fsize);
	Buffer[cc] = '\0';
	printsin( &from, "recv_udp: ", "Packet from:");
    printf("Got data : %s \n",Buffer);
    return_str=Buffer; 
    fflush(stdout);
sendto(socket_fd,(const char *)return_str, strlen(return_str),MSG_CONFIRM, (const struct sockaddr *) &from,fsize);
  }
